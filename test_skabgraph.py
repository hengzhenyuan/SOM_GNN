...
with open('SKABgraph_results.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Epoch', 'Graph Precision', 'Graph Recall', 'Graph F1'])

with open('SKABnode_results.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
...
for epoch in range(10):
    ...
    with open('SKABgraph_results.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([epoch + 1, graph_precision, graph_recall, graph_f1])

    # Append the node results to the node CSV file
    with open('SKABnode_results.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
